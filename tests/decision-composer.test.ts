import test from 'node:test';
import assert from 'node:assert/strict';
import { DecisionComposer } from '../src/modules/research/DecisionComposer.js';
import { RuleResult } from '../src/core/types/rule.js';

test('DecisionComposer (決策合成驗證): 應正確處理衝突與特殊狀態', (t) => {
  const composer = new DecisionComposer();
  const stockId = '2330';
  const asOf = '2024-04-03';

  // 1. 測試 BUY/SELL 衝突 (研究模式 !hasPosition)
  const conflictResults: RuleResult[] = [
    { ruleId: 'r1', ruleName: '多頭排列', category: 'entry', action: 'BUY', triggered: true, severity: 'info', reason: 'ok' },
    { ruleId: 'r2', ruleName: '乖離過高', category: 'exit', action: 'SELL', triggered: true, severity: 'warning', reason: 'high' }
  ];
  const conflictDecision = composer.compose({ stockId, asOf, ruleResults: conflictResults, thesisStatus: 'active', hasPosition: false });
  assert.strictEqual(conflictDecision.action, 'BLOCK', '研究模式下 SELL 應轉換為 BLOCK');
  assert.ok(conflictDecision.confidence < 0.6, '衝突應導致信心度下降');

  // 1.1 測試 BUY/SELL 衝突 (持倉模式 hasPosition)
  const positionDecision = composer.compose({ stockId, asOf, ruleResults: conflictResults, thesisStatus: 'active', hasPosition: true });
  assert.strictEqual(positionDecision.action, 'SELL', '持倉模式下應保留 SELL');

  // 2. 測試 thesisStatus = none
  const noThesisDecision = composer.compose({ stockId, asOf, ruleResults: [], thesisStatus: 'none' });
  assert.strictEqual(noThesisDecision.confidence, 0.4, '無論點時基礎信心度應為 0.4');
  assert.ok(noThesisDecision.summary.includes('[無論點支撐]'));

  // 3. 測試 broken -> BLOCK/EXIT 強制覆寫
  const brokenResults: RuleResult[] = [
    { ruleId: 'r1', ruleName: '強勢買入', category: 'entry', action: 'BUY', triggered: true, severity: 'info', reason: 'ok' }
  ];
  const brokenDecisionResearch = composer.compose({ stockId, asOf, ruleResults: brokenResults, thesisStatus: 'broken', hasPosition: false });
  assert.strictEqual(brokenDecisionResearch.action, 'BLOCK', '研究模式下論點破壞應強制 BLOCK');

  const brokenDecisionPosition = composer.compose({ stockId, asOf, ruleResults: brokenResults, thesisStatus: 'broken', hasPosition: true });
  assert.strictEqual(brokenDecisionPosition.action, 'EXIT', '持倉模式下論點破壞應強制 EXIT');

  // 4. 持倉模式下若無進出訊號但論點未破壞，應回傳 HOLD
  const holdDecision = composer.compose({
    stockId,
    asOf,
    ruleResults: [],
    thesisStatus: 'active',
    hasPosition: true,
    features: { totalScore: 72, institutionalNet: 100, closePrice: 105, ma20: 100, volumeRatio20: 0.9 },
    thesisEvaluation: {
      status: 'active',
      supportPasses: ['總分 >= 60', '法人未翻空'],
      supportFails: [],
      riskHits: [],
      disconfirmHits: [],
      recoverySignals: []
    }
  });
  assert.strictEqual(holdDecision.action, 'HOLD');
  assert.ok(holdDecision.summary.includes('維持持有'));
  assert.ok(holdDecision.summary.includes('論點訊號'));

  // 5. 持倉模式下 ADD / TRIM 應保留語意與條件說明
  const addDecision = composer.compose({
    stockId,
    asOf,
    ruleResults: [
      { ruleId: 'add', ruleName: 'Add On Strength', category: 'entry', action: 'ADD', triggered: true, severity: 'info', reason: 'ok' }
    ],
    thesisStatus: 'active',
    hasPosition: true,
    features: { totalScore: 85, institutionalNet: 300, closePrice: 110, ma20: 100, volumeRatio20: 1.4 }
  });
  assert.strictEqual(addDecision.action, 'ADD');
  assert.ok(addDecision.summary.includes('建議加碼'));
  assert.ok(addDecision.summary.includes('達成條件'));

  const trimDecision = composer.compose({
    stockId,
    asOf,
    ruleResults: [
      { ruleId: 'trim', ruleName: 'Trend Weakening', category: 'exit', action: 'TRIM', triggered: true, severity: 'warning', reason: 'weak' }
    ],
    thesisStatus: 'weakened',
    hasPosition: true,
    features: { totalScore: 65, institutionalNet: -120, closePrice: 95, ma20: 100, volumeRatio20: 0.8 }
  });
  assert.strictEqual(trimDecision.action, 'TRIM');
  assert.ok(trimDecision.summary.includes('建議減碼'));
  assert.ok(trimDecision.summary.includes('攔截條件'));

  const sellDecision = composer.compose({
    stockId,
    asOf,
    ruleResults: [
      { ruleId: 'sell', ruleName: 'Support Breakdown Sell', category: 'exit', action: 'SELL', triggered: true, severity: 'warning', reason: 'breakdown' }
    ],
    thesisStatus: 'weakened',
    hasPosition: true,
    features: { totalScore: 50, institutionalNet: -200, closePrice: 90, ma20: 100, bias20: -10 }
  });
  assert.strictEqual(sellDecision.action, 'SELL');
  assert.ok(sellDecision.explanation?.blockingConditions.some(item => item.includes('MA20')));
});
