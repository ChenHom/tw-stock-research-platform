...
test('DecisionComposer (決策合成驗證): 應正確處理衝突與特殊狀態', (t) => {
  ...
  const conflictDecision = composer.compose({ stockId, asOf, ruleResults: conflictResults, thesisStatus: 'active' });
  assert.strictEqual(conflictDecision.action, 'SELL', 'SELL 應優先於 BUY');
  ...
  // 3. 測試 broken -> EXIT 強制覆寫
  const brokenResults: RuleResult[] = [
    { ruleId: 'r1', ruleName: '強勢買入', category: 'entry', action: 'BUY', triggered: true, severity: 'info', reason: 'ok' }
  ];
  const brokenDecision = composer.compose({ stockId, asOf, ruleResults: brokenResults, thesisStatus: 'broken' });
  assert.strictEqual(brokenDecision.action, 'EXIT', '論點破壞時應強制 EXIT 即使有買入訊號');
});
