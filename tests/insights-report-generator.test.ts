...
test('InsightsReportGenerator: 應正確格式化優化建議 Markdown 報表', (t) => {
  ...
  const mockInsights: any = {
    ...
    topEffectiveRules: [{ ruleId: 'rule-high', accuracy: 0.9, hitCount: 10 }],
    lowEffectiveRules: [{ ruleId: 'rule-low', accuracy: 0.2, hitCount: 5 }],
    thesisPerformance: [{ status: 'thesis_met', accuracy: 0.45, count: 15 }],
    actionPerformance: [{ action: 'BUY', accuracy: 0.4, count: 10 }],
