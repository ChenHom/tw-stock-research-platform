...
test('CandidateResearchService (核心鏈路驗證): 應正確執行 TopN、錯誤容錯與計分排序', async (t) => {
  ...
  const service = new CandidateResearchService(mockScreeningService, mockPipeline, mockRunRepo);
...
test('CandidateResearchService (強制股票模式): 應直接研究指定 stockIds 並回傳 runId', async () => {
  const savedRuns: any[] = [];
  const mockScreeningService: any = {
    screen: async () => {
      throw new Error('強制股票模式不應觸發 screen');
    }
  };
  const mockPipeline: any = {
    run: async (input: { stockId: string }) => ({
      stockId: input.stockId,
      featureSnapshot: { payload: { totalScore: input.stockId === '6761' ? 55 : 30 } },
      finalDecision: { action: input.stockId === '6761' ? 'WATCH' : 'BLOCK', confidence: 0.5, summary: 'OK' },
      ruleResults: [],
      thesisStatus: input.stockId === '6761' ? 'weakened' : 'broken'
    })
  };
  const mockRunRepo: any = {
    save: async (run: any) => { savedRuns.push(run); },
    updateStatus: async () => {},
    saveResults: async () => {}
  };
  ...
  assert.ok(result.runId, '應建立正式 runId');
  assert.deepStrictEqual(result.results.map(item => item.stockId), ['6761', '1234'], '應依研究總分排序');
  assert.deepStrictEqual(savedRuns[0].criteria, { minVolume: 2000, forcedStocks: ['1234', '6761'] }, '應保留強制股票資訊於 run criteria');
});
