...
```
`latest` 會抓最近一筆 **completed** 任務；若要完全隔離同日批次，請優先使用 `detail <runId>`。
...
```
`latest` 同樣只會抓最近一筆 **completed** 任務；在 smoke / batch 驗證中建議直接傳入明確 `runId`。
...
```
`range` / `latest` 只會聚合 **completed** runs；若要避免不同批次互相污染，請改用 `runs`.
...
```
批次洞察建議一律使用 `runs <runId1,runId2,...>`，可確保樣本窗口與成效報表完全一致。
...
```
腳本會自動解析當次 `runId`，並用同一批 `runId` 串接 history / outcomes / performance / insights，避免 `latest` 漂移。
...
報表也會額外標示 `insufficient / observation / stability / actionable` 樣本階段，提醒目前能否解讀 insights。
